...
{ // another example of C++ inheritance
private:
...
int main()
{
    ...
    cout << "---\n";
    for (int i = 0; i < n; i++)
        cout << A->predecessor(arr[i]) << " " << arr[i] << " " << A->successor(arr[i]) << "\n";

    // deletion demo
    cout << "---\n";
    cout << "Current BST/AVL:";
    A->inorder();

    int deletionorder[] = {23, 100, 32, 71, 50, 7, 5, 1, 3, 6, 15, 4};
    for (int i = 0; i < n; ++i)
    {
        cout << "Deleting: " << deletionorder[i] << "\n";

        A->remove(deletionorder[i]);
        cout << "AVL, height: " << A->getHeight() << ", inorder traversal:";
        A->inorder();

        T->remove(deletionorder[i]);
        cout << "BST, height: " << T->getHeight() << ", inorder traversal:"; // equal or taller than A->getHeight()
